...
coverages <- bh(x)
plot(as.integer(names(coverages)),coverages, type = 'l', col ="red", lwd = 2, ylim = c(.8,1.2))
abline(h=1)
abline(v=1945)


#bhCoverageFromYearAll <-  function(codi, minA. = 10, AgeInt. = 5, minAges. = 8, ages., sex. = "f"){        ##  Data
#	
#	dif. <- codi$year2[1] - codi$year1[1]
#	
#	codi$birthdays            <- 0
...
#	}
#	
#	coverages <- unlist(lapply(agesFit, bhCoverageFromAges, codi = codi))
#	coverages
#}
#
#bhAll <- function(x, minA = 10, AgeInt = 5, minAges = 8, sex = "f"){
#	tab     <- data.frame(x)           ##  Dat in data frame : cod, age, pop1, year1, pop2, year2, death
#	tab1    <- split(tab,x$cod)
#	cods    <- unique(tab$cod)
#	ages    <- sort(unique(tab$age))
#	#minA. = minA;AgeInt. = AgeInt;minAges. = minAges;ages. = ages;sex. = sex
#	coverages <- parallel::mclapply(
#					tab1, 
#					bhCoverageFromYearAll, 
#					minA. = minA, 
#					AgeInt. = AgeInt, 
#					minAges. = minAges,  
#					ages. = ages,
...
#Cover <- matrix(unlist(coverages),ncol=10,byrow=TRUE)
#matplot(as.integer(names(coverages)),Cover, type = 'l', col = "00000050", lty = 1, ylim = c(.8,1.2))
